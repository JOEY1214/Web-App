...
$event_edit = get_event($_GET['i']);
if (is_null($event_edit)) {
    //如果文章是null就轉回列表頁
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><?php echo $event_edit['title']; ?></h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <form id="edit_event_form" role="form">
                                    <input type="hidden" id="id" value="<?php echo $event_edit['id']; ?>">
                                    <div class="form-group">
                                        <label>Title of event</label>
                                        <input id="title" class="form-control"
                                               value="<?php echo $event_edit['title']; ?>">
...
<!--                                               placeholder="--><?php //echo $event_edit['author']; ?><!--" disabled="">-->
<!--                                    </div>-->
                                    <div class="form-group">
                                        <label>Picture</label>
                                        ...
                                        <input type="hidden" id="image_path"
                                               value="<?php echo ($event_edit['image_path']) ? $event_edit['image_path'] : ''; ?>">
                                        <div class="show_image">
                                            <?php if ($event_edit['image_path'] && file_exists("../" . $event_edit['image_path'])): ?>
                                                <img src='<?php echo "../" . $event_edit['image_path']; ?>'>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <div class="form-group" id="editor">
                                        <h4>Content</h4>
                                        <textarea id="content">
                                            <?php echo $event_edit['content']; ?>
                                        </textarea>
                                    </div>

                                    <button type="submit" class="btn btn-default btn-lg">Save</button>
                                    <a href="events.php" class="btn btn-default btn-lg">Cancel</a>
                                </form>
                            </div>
    ...
    $(document).ready(function () {
        ...
        $("#edit_event_form").on("submit", function () {
            ...
            if ($("#title").val() == '' || $("#content").val() == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "../php/update_event.php", //因為此檔案是放在 admin 資料夾內，若要前往 php，就要回上一層 ../ 找到 php 才能進入 add_article.php
                    data: {
                        id: $("#id").val(), //使用者帳號
                        title: $("#title").val(), //使用者帳號
                        start_date: $("#datepicker").val(), //使用者帳號
